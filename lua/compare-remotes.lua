 ...
 local function set_remotes(remote_map) _G.assert((nil ~= remote_map), "Missing argument remote-map on fennel/src/compare-remotes.fnl:20")
 ...
 local function replace_scheme(path, replacements) _G.assert((nil ~= replacements), "Missing argument replacements on fennel/src/compare-remotes.fnl:28") _G.assert((nil ~= path), "Missing argument path on fennel/src/compare-remotes.fnl:28")
 ...
 local function get_remote_path(remote_prefix, path, dir_3f) _G.assert((nil ~= dir_3f), "Missing argument dir? on fennel/src/compare-remotes.fnl:35") _G.assert((nil ~= path), "Missing argument path on fennel/src/compare-remotes.fnl:35") _G.assert((nil ~= remote_prefix), "Missing argument remote-prefix on fennel/src/compare-remotes.fnl:35")
 ...
 local function open_diff(local_path, remote_prefix) _G.assert((nil ~= remote_prefix), "Missing argument remote-prefix on fennel/src/compare-remotes.fnl:40") _G.assert((nil ~= local_path), "Missing argument local-path on fennel/src/compare-remotes.fnl:40")
 ...
 local function entry_selected(local_path, _3fchoice) _G.assert((nil ~= local_path), "Missing argument local-path on fennel/src/compare-remotes.fnl:47")
 if _3fchoice then
 ...
 local function open_remote_selection(local_path) _G.assert((nil ~= local_path), "Missing argument local-path on fennel/src/compare-remotes.fnl:52")
 ...
 local function project_file_3f(path) _G.assert((nil ~= path), "Missing argument path on fennel/src/compare-remotes.fnl:57")
 return (not startswith(path, "/") and ("" ~= glob(path))) end

 local function project_file_scheme_3f(path) _G.assert((nil ~= path), "Missing argument path on fennel/src/compare-remotes.fnl:60")
 local scheme = string.match(path, "^(.+)://")
 return (not scheme or tbl_contains(project_file_schemes, scheme)) end

 local function buf_path__3erelative_project_path(buf_path) _G.assert((nil ~= buf_path), "Missing argument buf-path on fennel/src/compare-remotes.fnl:64")
 ...
 local function compare_file(path, _3fremote) _G.assert((nil ~= path), "Missing argument path on fennel/src/compare-remotes.fnl:70")
 if _3fremote then
